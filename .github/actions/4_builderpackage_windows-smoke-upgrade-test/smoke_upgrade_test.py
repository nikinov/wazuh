...
def install_package(package_path):
    ...
    # /l*v <file> to generate a verbose log
    cmd = ["msiexec", "/i", package_path, "/qn", "/norestart", "/q", "WAZUH_MANAGER=1.1.1.1", "/l*v", temp_log_path]
    try:
    ...
    if os.path.exists(temp_log_path):
        try:
            with open(temp_log_path, 'r', encoding='utf-8', errors='replace') as src:
                log_content = src.read()
            with open(upgrade_log, 'a', encoding='utf-8') as dst:
...
def verify_installation(expected_version):
    upgrade_log_path = r"C:\win-agent-base\upgraded_version.log"
    success_pattern = "Product: Wazuh Agent -- Installation completed successfully"

    try:
        if not os.path.exists(upgrade_log_path):
            print(f"Error: Upgrade log not found at {upgrade_log_path}")
            return False

        with open(upgrade_log_path, "r", encoding="utf-8", errors="replace") as f:
            log_content = f.read()
        print(log_content)
        ...
        subprocess.run([agent_exe, "start"], check=True)
